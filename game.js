...
const game = {
  ...
  // Removed: _tryWebsim (Pollinations deprecated)

  async _tryWebsim(prompt, { aspect_ratio = "4:3", seed, transparent = false }) {
    try {
      const p = websim.imageGen({ prompt, aspect_ratio, seed, transparent });
      const res = await withTimeout(p, 12000);
  ...
  // Optimized: unified, parallel, cached image generation (Websim only)
  ...
  async generateNpcResponse() {
    ...
    try {
      const completion = await websim.chat.completions.create({ messages: [historyForAI[0] || {role:"system", content:""}, bondCue, ...historyForAI.slice(1)] });
    ...
    return (async () => {
      try {
        const completion = await websim.chat.completions.create({
          messages: [{ role:"system", content:"Suggest up to 3 brief, empathetic therapist replies (2–12 words) aligned with reflective listening; JSON array of strings only." }, ...history],
          json: true,
        });
...
Provide a brief, one-paragraph therapeutic analysis connecting their interpretation to their core crisis and personality. Speak in a gentle, insightful tone. Focus on what their interpretation reveals about their psychological state and worldview. Do not give a diagnosis.`;
      ...
      const completion = await websim.chat.completions.create({ messages: [{ role: "system", content: systemPrompt }] });
...
Example: {"breakthrough": true, "summary": "The patient has accepted that their value is not defined by their function.", "item_prompt": "A single, glowing gear crafted from polished wood, sitting on a velvet cushion, representing newfound purpose."}`
    ...
    try {
      const completion = await websim.chat.completions.create({ messages: [analysisPrompt, ...this.conversationHistory.slice(1)], json: true });
  ...
  togglePatientBio(forceShow) {
    ...
    if (forceShow === false || isVisible) { bioWindow.style.display = 'none'; }
      ...
      contentEl.querySelector('#pbw-summarize').onclick = async () => {
        ...
        try {
          const completion = await websim.chat.completions.create({ messages: [{ role:'system', content: sys }] });
...
Respond with only a JSON object: {"bond_change": number, "reason": "A brief explanation for the change."}`;
    try {
      const completion = await websim.chat.completions.create({ messages: [{ role: "system", content: systemPrompt }], json: true });
      const result = JSON.parse(completion.content);
      ...
      this.updateTrustUI();
    } catch (error) { console.error("AI bond analysis failed. No bond change.", error); }
  },
  ...
  importNpcFromJson() {
    try {
      ...
      Promise.resolve().then(async () => {
        ...
        try { if (cardFile) uploadedCardUrl = await websim.upload(cardFile); } catch(_) { this.toast('Card photo upload failed.'); }
  ...
  async saveNpcEdit() {
    ...
    try {
      ...
      if (offFile) { try { officeUrl = await websim.upload(offFile); } catch(_) { this.toast('Office image upload failed.'); } }
